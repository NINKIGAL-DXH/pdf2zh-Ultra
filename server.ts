...
`;
  ...
  if (forceReinstall) {
    ...
    let testCmd = pdf2zhCmdLocal;
    let testArgs = ["--version"];
    if (!isWin && fs.existsSync(pdf2zhCmdLocal)) {
         testCmd = runtime.pythonCmd;
         testArgs = [pdf2zhCmdLocal, "--version"];
    } else if (!fs.existsSync(pdf2zhCmdLocal)) {
         testCmd = "pdf2zh";
    }
    const testProc = spawn(testCmd, testArgs);
